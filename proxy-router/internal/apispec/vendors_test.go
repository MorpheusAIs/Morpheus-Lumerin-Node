...
func TestGatedStack(t *testing.T) {
	for _, s := range []string{"venice", "openrouter", "litellm", "anthropic", "openai", "groq", "together", "gemini", "moonshot"} {
		require.True(t, gatedStack(s), s)
	}
	for _, s := range []string{"vllm", "sglang", "llamacpp", "ollama", "tgi", "lmstudio", "koboldcpp", ""} {
