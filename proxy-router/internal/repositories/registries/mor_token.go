...
// IncreaseAllowance increases the allowance for a spender by the given amount.
// Unlike Approve which sets the allowance, this adds to existing allowance,
// making it safe for concurrent use.
func (g *MorToken) IncreaseAllowance(ctx *bind.TransactOpts, spender common.Address, addedValue *big.Int) (*types.Transaction, *types.Receipt, error) {
	tx, err := g.mor.IncreaseAllowance(ctx, spender, addedValue)
	if err != nil {
		return nil, nil, lib.TryConvertGethError(err)
	}
...
// IncreaseAllowanceTx builds an increaseAllowance transaction without sending it.
// Use with opts.NoSend = true for escalation support.
func (g *MorToken) IncreaseAllowanceTx(opts *bind.TransactOpts, spender common.Address, addedValue *big.Int) (*types.Transaction, error) {
	tx, err := g.mor.IncreaseAllowance(opts, spender, addedValue)
	if err != nil {
		return nil, lib.TryConvertGethError(err)
	}
	return tx, nil
