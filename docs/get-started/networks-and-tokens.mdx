...
<Steps>
  <Step title="Pick a wallet">
    Any ERC-20 wallet works. If you import a mnemonic whose funded address is a derived sub-account (MetaMask "Account 2" and up), enter that account index during import — the default is index 0 — or add the account later from the wallet switcher (MorpheusUI 7.12+). Importing the private key directly avoids the question.
  </Step>
