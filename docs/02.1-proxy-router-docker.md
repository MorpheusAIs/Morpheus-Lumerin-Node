...
* The image is available at: [Morphues Node Package](https://github.com/orgs/Lumerin-protocol/packages?repo_name=Morpheus-Lumerin-Node) source for the different builds
...
docker run -d \
  ...
  -p 3333:3333 \
  -p 8082:8082 \
  ghcr.io/lumerin-protocol/proxy-router:latest
```
