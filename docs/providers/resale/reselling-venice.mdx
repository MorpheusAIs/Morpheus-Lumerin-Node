...
After edits, call `POST /config/models/reload` to pick up the change without cutting open sessions (see [Reload without a restart](/reference/models-config#reload-without-a-restart)); on releases before the endpoint existed, restart the proxy-router. Full register/bid contract steps: [Register on chain](/providers/full/register-onchain).
