...
The proxy-router no longer answers browser CORS preflights with `*`. Out of the box it accepts loopback origins, the desktop app, and the hosted `https://myprovider.mor.org`, so the table above works on a stock node. If you self-host MyProvider on another origin, add it to [`PROXY_CORS_ALLOWED_ORIGINS`](/reference/env-proxy-router#network-ports--urls) on the node (comma-separated). A local `npm run dev` on `localhost` needs nothing extra.
