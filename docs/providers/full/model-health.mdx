...
**Changed `models-config.json`?** Use `POST /config/models/reload` rather than restarting: it swaps the model table in place without cutting open sessions and queues this same sweep for you, so newly added models with a bid are probed immediately. See [Reload without a restart](/reference/models-config#reload-without-a-restart).
