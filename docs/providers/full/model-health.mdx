...
<Note>
The `httpStatus` and `modelName` report fields, the `rate_limited` error kind, and probe pacing (`MODEL_HEALTH_CHECK_PROBE_DELAY`) were added in the release **after** v7.4.0. On v7.4.0 nodes you get the report without those fields.
</Note>
...
**The report is cached, and there is no "run now" endpoint.** `lastChecked` (unix epoch — `date -r 1783536653` on macOS, `date -d @1783536653` on Linux) tells you how stale each entry is. After fixing a backend, either wait out the interval or **restart the proxy-router** — a sweep always runs at startup, so a restart doubles as a forced re-check.
