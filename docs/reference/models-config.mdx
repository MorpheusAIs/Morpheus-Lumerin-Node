...
description: "How the proxy-router maps on-chain model IDs to backend HTTP endpoints, including the apiType stack presets (OpenAI, vLLM, SGLang, llama.cpp, Ollama, Venice, OpenRouter, LiteLLM, Anthropic) and legacy Prodia / Hyperbolic adapters."
...
**`anthropic` preset transport:** the `anthropic` preset uses the Anthropic Messages transport; the request's `max_tokens` is forwarded (default 1024 when omitted) and `thinking.budget_tokens` must stay below it.
