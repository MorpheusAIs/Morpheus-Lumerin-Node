...
| `PROXY_CORS_ALLOWED_ORIGINS` | (unset) | Comma-separated list of extra origins allowed to call the HTTP API by browser. Loopback origins and Electron's `null`/`file` origins are always allowed regardless of this setting. Set to `*` to allow any origin (not recommended — the API exposes wallet operations such as `/blockchain/send/mor`); needed if you front the API with a non-localhost browser client. |
...
| Variable | Default | Notes |
|----------|---------|-------|
| `LLM_TIMEOUT` | `4m` | P-Node → backend LLM timeout (streaming + non-streaming). Set to an empty value (`LLM_TIMEOUT=`) to disable the deadline entirely for a genuinely slow model. |
| `CNODE_PNODE_TIMEOUT` | `90s` | Per-attempt timeout for C-Node waiting for P-Node first response. Set to an empty value (`CNODE_PNODE_TIMEOUT=`) to disable the deadline entirely. |
...
| `MODEL_API_DETECT_ENABLED` | `true` | Auto-detect the backend API of models without `apiStack` (service-endpoint probes, never inference requests, one hop through LiteLLM); `false` disables all probing and such models report no `api` block. Per-model budget: 10 s, capped by `MODEL_HEALTH_CHECK_TIMEOUT` |
