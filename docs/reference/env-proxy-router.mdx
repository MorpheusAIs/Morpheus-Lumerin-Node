...
| `ETH_NODE_ADDRESS` | (unset, falls back to built-in public RPC round-robin) | RPC endpoint URL (HTTPS or WSS — must match `ETH_NODE_USE_SUBSCRIPTIONS`). **Set explicitly for any real deployment** — the public round-robin fallback is rate-limited and unreliable; missed events break the node silently. The fallback rotates to the next endpoint on rate limits, `401`/`403` WAF pages, `404`/`405`/`410` (a retired service), and on read-only endpoints that answer `eth_sendRawTransaction` with `method is not allowed` — so opening a session works out of the box even when the first public URL refuses writes. |
...
| `ETH_NODE_MAX_RECONNECTS` | `30` | Max WSS resubscribe attempts (`-1` = unlimited). Retries back off exponentially from 0.5 s to a 30 s cap, and each attempt resubscribes from the block after the last event seen. |
...
| `PROXY_CORS_ALLOWED_ORIGINS` | (unset) | Comma-separated list of extra origins allowed to call the HTTP API by browser. Always allowed regardless of this setting: loopback origins, Electron's `null`/`file` origins, and the hosted [MyProvider](/providers/full/myprovider-gui) at `https://myprovider.mor.org`. Set to `*` to allow any origin (not recommended — the API exposes wallet operations such as `/blockchain/send/mor`); needed if you front the API with some other non-localhost browser client. |
| `MORPHEUS_DESKTOP_INSTANCE_TOKEN` | (unset) | Opaque identifier the Desktop app sets so multiple local proxy-router instances can be told apart. When set, it is echoed back on the `X-Morpheus-Instance-Token` header of `GET /healthcheck`. Not needed outside the bundled Desktop app. |
...
| `PROXY_STORE_CHAT_CONTEXT` | `true` | Persist chat context as one JSON file per chat under `<PROXY_STORAGE_PATH>/chats/`. The directory is kept at mode `0700` and files at `0600` (enforced on every read and write); writes are atomic (temp file + rename), so a crash mid-write cannot truncate a chat. Multimodal turns keep their `multi_content` parts (image / document attachments) in the stored history. **Frozen `false` in `-tee` images.** |
...
| Variable | Default | Notes |
|----------|---------|-------|
| `LLM_TIMEOUT` | `4m` | P-Node → backend LLM timeout (streaming + non-streaming). Go duration (`30s`, `10m`, `1h`). Empty or `0` **falls back to the default** — there is no "no deadline" setting; raise the value for a genuinely slow model. |
| `CNODE_PNODE_TIMEOUT` | `90s` | Per-attempt timeout for C-Node waiting for the P-Node's first response on chat / embeddings / media / audio. Empty or `0` falls back to the default. Total budget ≈ `CNODE_PNODE_TIMEOUT × CNODE_PNODE_MAX_RETRIES`. |
| *(fixed)* | `30s` | Read deadline on the C-Node → P-Node control calls that are not streamed: session handshake (`initiate_session`), provider ping, session reports, agent tool listing / calls, and audio upload chunks. Not configurable. A provider that accepts the TCP connection and then goes silent now fails with `504 provider did not respond within 30s` instead of hanging the request forever. |
...
## Gateway managed mode (optional)

Only for consumer nodes that sit **behind a companion gateway** (Uplink or similar) which opens and closes sessions on the node's behalf. Both variables are unset by default and the node behaves exactly as before. Full semantics: [Companion gateway mode](/prosumers/companion-gateway-mode).

| Variable | Default | Notes |
|----------|---------|-------|
| `GATEWAY_JOURNAL_PATH` | *(unset — disabled)* | Directory for per-operation journals. Setting it enables **managed mode**: the `X-Gateway-Operation` idempotency header, `progress` in open/close/withdraw responses, a process-local wallet lock, and the gateway stake formula (supply/budget refresh, +0.01% round-up, 300 s floor). Must be persistent storage; single replica only. |
| `GATEWAY_OWN_CLEANUP` | *(unset)* | Set to `1` **only** when the companion gateway itself closes expired sessions. Suspends the node's native expiry loop after boot-time rehydration. **With no live companion, expired sessions are never auto-closed.** Has no effect unless `GATEWAY_JOURNAL_PATH` is also set. |
...
| Variable | Default | Notes |
|----------|---------|-------|
| `TEE_PORTAL_URL` | `https://secretai.scrtlabs.com/api/quote-parse` | SecretAI Portal used to verify raw TDX CPU quotes |
