...
| `x-morpheus-history: off` | No | Skip storing this exchange and do not prepend stored history to it, for one request only. Useful for agent / tool calls that must not pollute a chat. `on` / `default` keep the node's `PROXY_STORE_CHAT_CONTEXT` / `PROXY_FORWARD_CHAT_CONTEXT` policy; the header cannot enable history the operator turned off. |
...
Vision-capable models accept the OpenAI multimodal shape — a `content` array of `{"type":"text"}` and `{"type":"image_url","image_url":{"url":"data:image/png;base64,…"}}` parts. The parts are forwarded to the provider and, when history is stored, kept in the saved chat. Whether a given model actually handles images is up to its provider.
...
Structured decision / System One models use a separate route — see [Decisions](/consumers/decisions) (`POST /v1/decisions`). Do not send Decisions bodies to `/v1/chat/completions`.
