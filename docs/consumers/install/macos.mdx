...
<Warning>
**Wallet:** if you import an existing MetaMask mnemonic, MorpheusUI derives **account index 0** by default. If your funded address is "Account 2" or later, set the matching index (Account N = index N−1) in the import screen, or add it afterwards from the wallet switcher (MorpheusUI 7.12+). Importing the private key directly sidesteps this.
</Warning>
