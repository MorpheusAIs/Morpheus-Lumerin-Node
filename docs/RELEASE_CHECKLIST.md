...
- [ ] [`README.md`](../readme.md) — same table sanity-check.
...
- [ ] Confirm [`proxy-router/docs/swagger.yaml`](../proxy-router/docs/swagger.yaml) is up to date (auto-regenerated by the CI/CD build pipeline via `swag init`).
...
- [ ] No pages have `last_verified` older than two minor releases (the [docs CI workflow](../.github/workflows/docs.yml) raises warnings).
...
- [ ] [`AGENTS.md`](../AGENTS.md) lookup table still points at correct pages.
- [ ] [`.cursor/rules/morpheus.mdc`](../.cursor/rules/morpheus.mdc) rules still reflect current behavior.
