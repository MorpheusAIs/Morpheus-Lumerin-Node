...
MorpheusUI's mnemonic import derives **account index 0** unless you tell it otherwise. If your real wallet was a derived sub-account (e.g. MetaMask "Account 2" = index `1`), recovering by mnemonic at the default index gives you a different address with no funds. Fix: enter the right index in the import screen, or — on MorpheusUI 7.12+ — open the wallet switcher and **add an HD account**; the node re-derives from the stored phrase (`POST /wallet/derivationPath`) without you re-typing it. Importing the private key directly also works.
