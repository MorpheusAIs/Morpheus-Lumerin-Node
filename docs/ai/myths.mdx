...
<AccordionGroup>
  ...
  <Accordion title="MYTH: There is a 'recover' RPC for stuck sessions.">
    **No.** There is no single `recover` call. **`closeSession`** stops the session and triggers refund logic. **`withdrawUserStakes`** separately claims day-locked balances (`userStakesOnHold`). The proxy-router exposes those actions as `POST /blockchain/sessions/:id/close` and `POST /blockchain/stakes/withdraw`; the latter can also be called directly on the Diamond contract.
  </Accordion>
  ...
  <Accordion title="MYTH: Recover from mnemonic always works.">
    **Only if you pick the right account index.** A mnemonic import in MorpheusUI defaults to index 0. If your MetaMask address was a derived sub-account ("Account 2" = index `1`), recovering at the default lands you on a different address with no funds. Set the index during import, add the HD account from the wallet switcher afterwards (MorpheusUI 7.12+, `POST /wallet/derivationPath`), or import the **private key** instead.
  </Accordion>
