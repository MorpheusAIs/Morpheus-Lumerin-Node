...
export const StakeAdd = (props: StakeAddProps) => {
  ...
  const { writeContractAsync, data, isPending, isError, error } = useWriteContract();
  ...
  return (
    <div>
      ...
      <input
        ...
        id="stakeAmount"
        name="stakeAmount"
        onChange={(e) => setStakeAmount(parseUnits(e.target.value, decimal.data))}
      />

      <select value={lockIndex} onChange={(e) => setLockIndex(Number(e.target.value))}>
        {locks.data?.map((lock, index) => (
          <option key={lock.durationSeconds} value={index}>
            Duration {formatDuration(lock.durationSeconds).toString()}, multiplier{" "}
            {Number(lock.multiplierScaled) / Number(props.precision)}
          </option>
        ))}
      </select>
