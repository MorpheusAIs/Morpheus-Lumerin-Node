...
export const openLogFile = (name, retry = true) => {
  try {
...
export const isProxyRouterHealthy = async (api, url) => {
  try {
...
export const runProxyRouter = (config) => {
  try {
    const resourcePath =
    ...
    const ls = spawn(`${resourcePath}/executables/proxy-router`, proxyConfig, {
      detached: true,
      stdio: ['ignore', out, err]
    })

    logger.error(
      `This is not error, but important info. Proxy config: ${JSON.stringify(proxyConfig.filter((c) => !c.includes('private-key')))}`
    )
...
export default { runProxyRouter, isProxyRouterHealthy }
