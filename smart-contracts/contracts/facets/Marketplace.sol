...
contract Marketplace {
  ...
  function getActiveBidsByModelAgent(bytes32 modelAgentId) external view returns (bytes32[] memory, Bid[] memory) {
    ...
    /// @notice returns active bids by model or agent id
  function getActiveBidsRatingByModelAgent(bytes32 modelAgentId, uint256 offset, uint8 limit) external view returns (bytes32[] memory, Bid[] memory, ProviderModelStats[] memory, ModelStats memory) {
    KeySet.Set storage modelAgentBidsSet = s.modelAgentActiveBids[modelAgentId];
    uint256 length = modelAgentBidsSet.count();
    ModelStats memory modelStats = s.modelStats[modelAgentId];

    if (length < offset) {
      return (new bytes32[](0), new Bid[](0), new ProviderModelStats[](0), modelStats);
    }
