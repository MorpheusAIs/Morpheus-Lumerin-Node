...
describe.skip("Staking", function () {
  it("should stake", async function () {
    ...
    expect(
      await sessionRouter.read.withdrawableStakeBalance([
        expectedStake.account,
      ]),
    ).equal(expectedStake.stakeAmount);
  });
  ...
  describe.skip("Stipend", function () {
    ...
    it("should spend daily stipend", async function () {
      const { sessionRouter, expectedStake, tokenMOR } =
      ...
      expect(
        await sessionRouter.read.balanceOfDailyStipend([expectedStake.account]),
      ).eq(expectedStake.expectedStipend - expectedStake.spendAmount);
    });
    ...
    it("should count spend amount correctly when spent second time same day", async function () {
      ...
      expect(
        await sessionRouter.read.balanceOfDailyStipend([expectedStake.account]),
      ).eq(expectedStake.expectedStipend - expectedStake.spendAmount * 2n);
    });
    ...
    it("should reset daily stipend value on the next day", async function () {
      ...
      expect(
        await sessionRouter.read.balanceOfDailyStipend([expectedStake.account]),
      ).eq(expectedStake.expectedStipend);
    });
    ...
    it("should lock unstaking till next day after using stipend", async function () {
      ...
      const withdrawableStakeBefore =
        await sessionRouter.read.withdrawableStakeBalance([
          expectedStake.account,
        ]);
      await time.increase(DAY / SECOND);
      const withdrawableStakeAfter =
        await sessionRouter.read.withdrawableStakeBalance([
          expectedStake.account,
        ]);
    ...
    it("should return stipend to the staking contract and increase daily stipend", async function () {
      ...
      const stipendAfterTransfer =
        await sessionRouter.read.balanceOfDailyStipend([expectedStake.account]);

      await tokenMOR.write.approve(
        [sessionRouter.address, expectedStake.spendAmount],
        {
          account: expectedStake.transferTo,
        },
      );
      await sessionRouter.write.returnStipend(
        [expectedStake.account, expectedStake.spendAmount],
        {
          account: expectedStake.transferTo,
        },
      );
      ...
      expect(stipendAfterTransfer).eq(
        stipendBeforeTransfer - expectedStake.spendAmount,
      );
