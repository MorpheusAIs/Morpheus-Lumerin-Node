...
describe("Provider registry", function () {
  describe("Deployment", function () {
    ...
    //   const { providerRegistry, owner, marketplace } = await loadFixture(deployMarketplace);
  ...
  describe("Actions", function () {
    it("Should register", async function () {
      const { providerRegistry, provider, expectedProvider } = await loadFixture(
        deploySingleProvider
      );
      const data = await providerRegistry.read.providerMap([provider.account.address]);
    ...
    it("Should error when registering with insufficient allowance", async function () {
      const { providerRegistry, provider, tokenMOR } = await loadFixture(deployDiamond);
      try {
        await providerRegistry.simulate.providerRegister([
          provider.account.address,
          100n,
    ...
    it("Should error when register account doesnt match sender account", async function () {
      ...
      const { providerRegistry, provider, tokenMOR, owner } = await loadFixture(deployDiamond);
      try {
        await providerRegistry.simulate.providerRegister([user.account.address, 100n, "endpoint"], {
    ...
    it("Should deregister by provider", async function () {
      ...
      await providerRegistry.write.providerDeregister([provider.account.address], {
      ...
      expect(await providerRegistry.read.providerGetCount()).eq(0n);
      expect((await providerRegistry.read.providerMap([provider.account.address]))[3]).to.equal(
        true
      );
      expect(events.length).eq(1);
    ...
    it("Should return stake on deregister", async function () {
      const { providerRegistry, provider, tokenMOR, expectedProvider } = await loadFixture(
        deploySingleProvider
      );
    ...
    it("Should update stake and url", async function () {
      const { providerRegistry, provider, tokenMOR, expectedProvider, publicClient } =
      ...
      const providerData = await providerRegistry.read.providerMap([provider.account.address]);

      expect(providerData).deep.equal([
        updates.endpoint,
        expectedProvider.stake + updates.addStake,
        timestamp,
        expectedProvider.isDeleted,
      ]);
    });
  ...
  describe("Getters", function () {
    it("Should get by index", async function () {
      ...
      const [address, providerData] = await providerRegistry.read.providerGetByIndex([0n]);
  ...
  describe("Views", function () {
    it("should get all", async function () {
      const { providerRegistry, expectedProvider } = await loadFixture(deploySingleProvider);
  ...
  describe("Min stake", function () {
    it("Should set min stake", async function () {
      ...
      await providerRegistry.write.providerSetMinStake([minStake], { account: owner.account });
    ...
    it("Should error when not owner is setting min stake", async function () {
      ...
      try {
        await providerRegistry.write.providerSetMinStake([100n], { account: provider.account });
        expect.fail("Expected error");
      } catch (e) {
        expectError(
          e,
