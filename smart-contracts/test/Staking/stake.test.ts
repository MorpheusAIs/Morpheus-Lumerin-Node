...
describe('Staking contract - stake', () => {
  ...
  describe('Actions', () => {
    ...
    it('should error if staking before start date', async () => {
      ...
      const newPoolId = await staking.addPool.staticCall(startTime, duration, totalReward, [DAY], [pool.precision]);
      await staking.addPool(startTime, duration, totalReward, [DAY], [pool.precision]);

      const stakeAmount = 1000n;
      await LMR.connect(ALICE).approve(staking, stakeAmount);

      await expect(staking.connect(ALICE).stake(newPoolId, stakeAmount, 0)).to.be.revertedWithCustomError(
        staking,
        'StakingNotStarted',
      );
    ...
    it('Should error if staking duration is too long', async () => {
      ...
      await expect(staking.connect(ALICE).stake(poolId, stakingAmount, lockDuration)).to.be.revertedWithCustomError(
        staking,
    ...
    it('should error if not enough allowance', async () => {
      await expect(staking.connect(ALICE).stake(poolId, stakingAmount, lockDuration)).to.be.revertedWith(
